...
import { openBrandMenu, openModelMenu } from "../recoil/atoms";
import { Link } from "react-router-dom";

const Header = () => {
    const mainList = ["모델", "구매", "체험", "멤버스", "제네시스"];
    const userList = ["고객센터", "마이 페이지"];
    const isSetMenuOpen = useSetRecoilState(openModelMenu);
    const isSetBrandOpen = useSetRecoilState(openBrandMenu);
    const menuClick = (e) => {
        if(e.target.innerHTML === "모델"){
            isSetMenuOpen((prev)=>!prev);
            isSetBrandOpen(false);
        }else if(e.target.innerHTML === "제네시스"){
            console.log("genesis");
            isSetBrandOpen((prev)=>!prev)
            isSetMenuOpen(false);
        };
    };
